...
torch::Tensor toTensorDouble(containers::matrix &m) {
    return torch::from_blob(m.data(), {static_cast<int64_t>(m.rows), static_cast<int64_t>(m.cols)},
                            torch::kFloat64)
        .clone();
}

torch::Tensor toTensorInt8(containers::Vector1D &v) {
    return torch::from_blob(v.data(), {static_cast<int64_t>(v.size())}, torch::kInt8).clone();
}
